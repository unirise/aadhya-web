...
function RolesByIntelligenceChart({ data = {}, height = 400 }) {
  ...
  const chartData = Object.entries(data).map(([intelligence, roles]) => ({
    name: intelligence,
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          <p style={{ margin: 0, fontWeight: 'bold', color: '#1f2937', marginBottom: '8px' }}>
            {data.name} Intelligence
          </p>
          <p style={{ margin: 0, fontSize: '16px', fontWeight: 'bold', color: '#3b82f6', marginBottom: '8px' }}>
          ...
          {data.roles && data.roles.length > 0 && (
            <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid #e5e7eb' }}>
              ...
              {data.roles.slice(0, 3).map((role, idx) => (
                <p key={idx} style={{ margin: '2px 0', fontSize: '11px', color: '#4b5563' }}>
                  • {role}
                </p>
              ))}
              {data.roles.length > 3 && (
                <p style={{ margin: '2px 0', fontSize: '11px', color: '#9ca3af', fontStyle: 'italic' }}>
