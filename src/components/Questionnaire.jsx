...
function Questionnaire() {
  ...
  const fetchIntelligenceScores = useCallback(async () => {
    try {
      const intelligencesResponse =
        await questionsApi.fetchIntelligences(PERSON_ID)
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        // Load metadata, scoring, answer options, questions, and intelligence scores in parallel
        const [
          metadataResponse,
          scoringResponse,
          ...
          questionsApi.fetchMetadata(),
          questionsApi.fetchScoring(),
          questionsApi.fetchAnswerOptions(),
          questionsApi.fetchQuestions(5),
          questionsApi.fetchIntelligences(PERSON_ID),
        ])
  ...
  const loadMoreQuestions = useCallback(async () => {
    ...
    try {
      ...
      const pageResponse = await questionsApi.fetchQuestions(5, nextPage)

      // Unwrap response (backend wraps it in { status, message, data, stack })
      const pageData = pageResponse.data || pageResponse

      const newQuestions = pageData.questions || []
      if (newQuestions.length > 0) {
        setActivities(prev => [...prev, ...newQuestions])
        setCurrentPage(nextPage)
        // Backend returns random questions, so always allow loading more
        // Only stop if we get no questions back
      ...
      setError(err.message || 'Failed to load more questions.')
      setHasMorePages(false)
    } finally {
      setIsLoading(false)
    }
  ...
  // Load more questions when approaching the end (infinite scroll)
  useEffect(() => {
    // Load more when we're at or past the last question, or when we have 2 or fewer remaining
    const needsMoreQuestions =
      currentStep >= activities.length - 1 ||
      activities.length - currentStep <= 2
    if (needsMoreQuestions && !isLoading) {
      loadMoreQuestions()
    }
  }, [currentStep, activities.length, isLoading, loadMoreQuestions])
  ...
  const handleNext = async () => {
    if (answers[currentActivity.id]) {
      // If we're at the last question, try to load more first
      if (currentStep >= activities.length - 1 && !isLoading) {
        await loadMoreQuestions()
      }
      // Always advance to next question (safety check will prevent out-of-bounds)
